...
**対象アプリ**  
**Kernie**（製品名）。複数の期間付き活動（プログラム）の下でプロジェクトを束ね、タスクを GTD 的に扱う個人向けツール。最初は**無料**で**自分のみ**利用。最終的なクライアント形態は**まず Web**（将来 PWA や Tauri などのデスクトップ化はオプション）。
...
- 2026-04-24: 製品名を **Kernie** とする旨を明記。
